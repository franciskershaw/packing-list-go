...
- **`requests/auth.http` doesn't cover Google login/callback.** That flow
  needs a real browser round-trip and can't be driven by a plain `.http`
  request. `/auth/refresh` and `/auth/logout` *are* covered (PACK-027) —
  seeded via a dev refresh token/cookie from `scripts/gen_token.go`,
  relying on REST Client's cookie jar to chain the sequence.
